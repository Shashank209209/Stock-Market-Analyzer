...
A visual wrapper around stock_analyzer_v01.py's rule-based agents and
optional AI Synthesizer. Run with:
...
from stock_analyzer_v01 import (
    ...
    get_ai_synthesizer_recommendation,
)
...
# VIOLET marks the AI Synthesizer section specifically, visually
# separating "verified numbers" from "LLM reasoning over those numbers" -
# the same distinction that's the whole thesis of this project.
#
...
with st.sidebar:
    ...
    st.divider()
    enable_ai = st.checkbox("Enable AI Synthesizer Agent")
    api_key = None
    if enable_ai:
        api_key = st.text_input(
            "Anthropic API key",
            type="password",
            help="Get one at console.anthropic.com. Never shared or stored - used only for this session.",
        )
        st.caption("Your key is used only in this browser session and is never saved to disk.")
...
if analyze_clicked:
    ...
    # ------------------------------------------------------------------
    if enable_ai:
        st.markdown('<div class="section-accent violet"></div>', unsafe_allow_html=True)
        st.subheader("AI Synthesizer Agent")
        if not api_key:
            st.info("Enter your Anthropic API key in the sidebar to run the AI Synthesizer.")
        else:
            with st.spinner("Running AI Synthesizer Agent..."):
                try:
                    ai_output = get_ai_synthesizer_recommendation(
                        technical_recommendation, fundamental_recommendation,
                        technical_summary, fundamentals_summary, ticker,
                        api_key=api_key,
                    )
                    final_decision = extract_field(ai_output, "FINAL DECISION:")
                    conviction = extract_field(ai_output, "CONVICTION SCORE:")
                    alert_fn, emoji = decision_style(final_decision)
                    alert_fn(f"{emoji} **{final_decision}**  •  Conviction: {conviction}")
                    st.markdown(ai_output)
                except Exception as e:
                    st.error(f"AI Synthesizer failed: {e}")
    ...
    if enable_backtest:
        ...
        with st.spinner(f"Running backtest over {backtest_period}..."):
            try:
                bt = run_technical_backtest(ticker, backtest_period)
